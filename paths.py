...
for i in range(12,90):
    with open('/Users/peytonzhu/Desktop/research/ddb1_v02_01/total/total{}.txt'.format(i)) as f:
        line=f.readlines()
        for values in line:
    ...
    listX=randomListX(0,1500,80)
    listY=randomListY(0,1152,80)
    print(listX)
    print(listY)
    ...
    def pass_mask(mask, img):
        ...
        return qwe
    imgs=cv2.imread('/Users/peytonzhu/Desktop/GreenChannel/G{}.png'.format(i))
    ret0, th0 = cv2.threshold(i, 0, 255, cv2.THRESH_BINARY)
    mask = cv2.erode(th0, np.ones((7, 7), np.uint8))
    j=pass_mask(mask,imgs)

    for k in range(0,80):
        cropped = j[listY[i] - 13:listY[i] + 13, listX[i] - 13:listX[i] + 13]
        save_dir="/Users/peytonzhu/Desktop/Normal/normal{}_{}.png".format(i)(k)
